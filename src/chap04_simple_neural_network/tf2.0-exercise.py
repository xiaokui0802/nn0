...
# 比较自定义的 softmax 函数结果和tf自带的结果，误差小于 0.0001 则认为相等
...
def sigmoid(x):
    ##########
    '''实现 sigmoid 函数， 不允许用tf自带的 sigmoid 函数'''
    x = tf.cast(x, tf.float32) #将输入x转换为 float32 类型，确保数值计算的精度和类型一致性。
...
def softmax_ce(logits, label):
    ...
    #参数logits: 未经 Softmax 的原始输出（logits）
